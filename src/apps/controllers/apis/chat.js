...
2b. Nếu tìm thấy đủ → trả lời đầy đủ .
...
## NGUYÊN TẮC BẤT BIẾN
- Phải trả lời bằng tiếng Việt có dấu
...
const getConversation = async (chatId, userId, documentId) => {
    let conv = await ChatModel.findOne({ userId, _id: chatId, documentId });
    return conv;
};
...
exports.chatHybrid = async (req, res) => {
    ...
    try {
        ...
        conversation = await getConversation(chatId, userId, documentId);
        if (!conversation) {
        ...
        const cacheKey = `chat:${userId}:${documentId}:${crypto.createHash("sha256").update(query.toLowerCase()).digest("hex").slice(0, 32)}`;
        try {
        ...
        const [elasticResults, vectorResults] = await Promise.all([
            ...
            vectorStore.similaritySearch(searchQuery, TOP_VECTOR, { document_id: documentId }),
        ]);
...
exports.getAllConversations = async (req, res) => {
    try {
        const documentId = req.params.documentId;
        const userId = req.user.id;
        let conversations = await ChatModel.find({ userId, documentId }).sort({ lastActiveAt: -1 });
...
exports.getChatHistory = async (req, res) => {
    try {
        ...
        const userId = req.user.id;
        const documentId = req.params.documentId;
        const conversation = await ChatModel.findOne({ userId, documentId, _id: chatId });
        if (!conversation) {
